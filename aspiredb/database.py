...
#---------------------- LOCALS ------------------------------------------
from aspiredb.core.master_controller import Master, writeJsonData, updateIndex
from aspiredb.core.slave_controller import Slave, ugen
...
class Create( Setup ):
    ...
    async def create_database(self, dbname:str=None, access:str="public", password:str=None):
        '''POST'''
        if dbname:
            result = await self.master.create_slave(dbname, access=access, password=password)
            if type(result) == bool:
                if result:
    ...
    async def create_document(self, database:str=None, data:dict=None):
        '''POST'''
        if data:
            result = await self.slave.create_doc(database, data=data)
    ...
    async def create_private_document(self, database:str=None, data:dict=None, password:str=None):
        '''POST'''
        if data:
            result = await self.slave.create_doc(database, data=data, password=password)
...
class Clone( Setup ):
    ...
    async def clone_doc(self, dbname:str=None, doc_id:str=None, clone_id:str=None):
        """GET"""
        result = await self.slave.clone_doc(dbname=dbname, doc_id=doc_id, clone_id=clone_id)
...
class Read( Setup ):
    ...
    async def get_database(self, dbname:str=None, password:str=None):
        '''GET'''
        pass
    ...
    async def get_document(self, dbname:str=None, doc_id:str=None, password:str=None):
        ...
        if dbname and doc_id:
            if '.json' in doc_id:
                return json.dumps( await self.slave.get_document(slave=dbname, doc=doc_id, password=password))
            doc_id = f"{doc_id}.json"
            return json.dumps(await self.slave.get_document(slave=dbname, doc=doc_id, password=password))
        return json.dumps({"status": "A databas name and document id to retreive is required."})


    async def get_documents(self, dbname:str=None):
            '''GET'''
            if dbname:            
                return json.dumps( await self.slave.get_documents(slave=dbname))
...
class Update( Setup ):
    ...
    async def update_database(self, dbname:str=None, data:dict=None):
        '''PUT'''
        pass

    async def update_document(self, dbname:str=None, doc_id:str=None, data:dict=None):
        '''PUT'''
        if dbname and doc_id and data:
            result = await self.slave.update_doc(dbname=dbname, doc_id=doc_id, data=data)
            return json.dumps(result)
        else:
...
class Rollback( Setup ):
    ...
    async def rollback_delete_database(self, dbname:str=None):
        '''DELETE'''
        pass

    async def rollback_delete_document(self, doc_id:str=None):
        '''DELETE'''
        pass

    async def rollback_update_database(self, dbname:str=None):
        '''DELETE'''
        pass

    async def rollback_update_document(self, doc_id:str=None):
        '''DELETE'''
        pass
