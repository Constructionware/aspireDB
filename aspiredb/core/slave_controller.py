...
#------------------------ LOCAL DEPENDENCIES -----------------------------------
from aspiredb.core.master_controller import timestamp, writeJsonData, updateIndex, Master  
from aspiredb.core.encriptor import EncryptFile, EncryptMessage, GenerateId
...
class  Slave:
    ...
    async def create_doc(self, args, **kwargs):
        ...
        if "password" in kwargs.keys():
            ...
            return await self.__create_document(args, data=kwargs.get('data'), password=kwargs.get('password'))           
        else:  
            return await self.__create_document(args, data=kwargs.get('data'))             

    async def __create_document(self, args, **kwargs):
        ...
        try:
            ...
            await self.__update_slave_index(args, data=stub)           
            del(data) # clean up               
    ...
    async def get_document(self, args=None, **kwargs): 
    ...
    async def get_documents(self, args=None, **kwargs): 
        ...
        try:
            if isdir(handle):                           
                ...
                data = [await self.get_document(slave=kwargs.get('slave'), doc=item) for item in data]
                del(handle)
                return data
            return {'status': 'file not found'}                      
        except Exception as e: 
    ...
    async def update_doc(self, args=None, **kwargs):
    ...
    async def __update_document(self, **kwargs):
    ...
    async def clone_doc(self, **kwargs):
        ...
        result = await self.__clone_document(dbname=dbname, doc_id=doc_id, clone_id=clone_id)
        return result

    async def __clone_document(self, dbname:str=None, doc_id:str=None, clone_id:str=None):
        if '.json' in doc_id:
        ...
        handle = self.handle(slave=dbname, doc=doc_id)
        print(handle)
        base_doc = await self.get_document(slave=dbname, doc=doc_id)
        clone = base_doc.copy()
        clone["_id"] = clone_id
        clone['meta_data'] = {
            "cloned": {
        ...
        status = await self.__create_document(dbname, data=clone)
